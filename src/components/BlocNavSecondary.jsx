...
const BlocNavSecondary = ({smaller}) => {
    ...
    return (
        <nav  aria-label="Navigation secondaire" className={`blocNavSecondary ${smaller ? 'smaller' : ''}`}>
            ...
            {(location.pathname !== '/portfolio' && !location.pathname.startsWith('/portfolio/')) ? <Link className='cta__primary' to='/portfolio'><span>Mon super portfolio</span><img src="/img/btnArrow.webp" alt="fleche bouton" /></Link>  : ''}
